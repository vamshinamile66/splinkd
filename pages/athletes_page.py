...
class AthletesPage:
    ...
    def onboard_athlete(self, first_name, last_name, dob, gender, email, mobile):
        # 1. Click on athletes tab
        self.wait.until(EC.element_to_be_clickable((By.XPATH, "//a[contains(text(), 'Athletes')]"))).click()
        # 2. Click on 'on board athletes' button
        self.wait.until(EC.element_to_be_clickable((By.XPATH, "//button[contains(text(), 'On Board Athletes')]"))).click()
        # 3. Enter first name
